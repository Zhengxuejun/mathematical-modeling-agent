...
完整流程只有在 `final_ready` 和 `competition_ready` 均通过后才发布 `07_提交包` 并推进到有效 S8。`completed` 还要求本轮 finalizer 成功；历史 S8、旧 manifest 或旧 readiness JSON 不会把失败/跳过门禁的重跑误报为已完成或继续显示 `competition_ready=true`。开放 P0/P1、模板内容、未冻结或漂移的正式 run 产物、项目外证据、失败 manifest、文件缺失或 SHA256 不一致都会阻断交付。
