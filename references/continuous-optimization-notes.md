...
## 19. 当前已实现：离线建模能力 Benchmark Harness v1

已实现：

```text
scripts/modeling_benchmark.py
scripts/benchmark/
benchmarks/cases/
benchmarks/fixtures/
```

目标：把后续能力优化从“组件测试通过”提升为可比较的建模失败模式回归。v1 使用三个原创合成微型案例，分别锁定优化解不可行、同主体预测数据泄漏和多指标排序缺少稳定性分析；每类包含 good、bad、partial fixture。
